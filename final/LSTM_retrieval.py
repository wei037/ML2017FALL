...
def create_model():
    ...
    mfc_ouput = Bidirectional(LSTM(256 , return_sequences=True , dropout=0.15, recurrent_dropout=0.15))(mfc_input)
    mfc_ouput1 = Bidirectional(LSTM(128 , dropout=0.15, recurrent_dropout=0.15))(mfc_ouput)

    chi_input = Input(shape = (13,300))
    chi_ouput = Bidirectional(LSTM(256 , return_sequences=True , dropout=0.15, recurrent_dropout=0.15))(chi_input)
    chi_ouput1 = Bidirectional(LSTM(128 , dropout=0.15, recurrent_dropout=0.15))(chi_ouput)

    encoder_output = Dot(axes = 1 , normalize = True)( [mfc_ouput1 , chi_ouput1] )
...
cb = []
cb.append(ModelCheckpoint('model/model-loss_{val_loss:.5f}-acc_{val_acc:.5f}.h5', monitor='val_loss', save_best_only=True))

print ('cb done')
#model = create_model()
model = load_model('model/model-loss_0.59091-acc_0.70149.h5')
model.compile(loss= 'binary_crossentropy', optimizer=Adam() , metrics = ['accuracy'])
model.fit([mfc_train , chi_train], label_train , batch_size=384, epochs=100 , validation_split = 0.1 , callbacks=cb)
